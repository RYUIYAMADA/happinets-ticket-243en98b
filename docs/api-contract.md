...
| `adminLogin` | `POST /api/auth/admin-login` | PBKDF2検証・429 LOCKED 応答あり。Bearer token 発行 |
...
- Worker 側で PBKDF2（SubtleCrypto deriveBits・iterations=100000・salt=env.ADMIN_SALT）で再検証
...
- 401: ハッシュ不一致。失敗のたびに admins.failed_count をインクリメント
